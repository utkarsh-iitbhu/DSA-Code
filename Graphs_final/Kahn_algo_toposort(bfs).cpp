...
void dfs(int u, vector<bool>&vis,vector<ll>g[],queue<int> &q){
    vis[u] = true;
    for(auto ch:g[u]){  
        if(!vis[ch]){
            dfs(ch,vis,g,q);
...
vector<int> ans; 
int cntOfNodesForInvalidTOPOSort = 0 ;
    while(!q.empty()){
        int l = q.size();
        for(int i=0;i<l;i++){
            int u = q.front();
            ans.push_back(u);
            cntOfNodesForInvalidTOPOSort++;
            q.pop();
            for(auto ch:g[u]){
                indegree[ch]--;
                if(indegree[ch]==0){
    ...
    cout<<endl;
    if(cntOfNodesForInvalidTOPOSort!=n){
        cout<<"IMPOSSIBLE"<<endl;
    }
    else
    cout<<"If valid it is equal to n : "<<cntOfNodesForInvalidTOPOSort<<endl;
