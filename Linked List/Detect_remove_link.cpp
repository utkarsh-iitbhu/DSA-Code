...
Node* get_loop_start(Node* &head){
    ...
    // Delete start 
    Node* del = slow; // del is the node to be deleted
    while(del->next != slow){
        del = del->next;
    }
    del->next = NULL;
...
void solve(){
    ...
    cout<<detectLoop(node2)<<endl;

    // cout<<"Detect loop :"<<endl;
    // cout<<detect_cycle(node2)<<endl;

    cout<<"Loop is removed: "<<endl;
    Node* t  = get_loop_start(node2);
    printll(t);
    cout<<endl;
