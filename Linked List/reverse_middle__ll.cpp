...
Node* recursion_op(Node* &head){
    // base case
    if(!head || !head->next){
        return head;
    }
    // Ye baaki ke ll ke liye hai 
    Node *chotahead = recursion_op(head->next); // chotahead is assigned to head->next
    // Ulta kar rahe hai pointers ko 
    head->next->next = head; // head->next->next is assigned to head
    // Ab reverse kar rahe hai like first node ko reverse kar diya 
    head->next = NULL; // head->next is assigned to NULL

    // Chota head me rahega ans as wo hi to sab reverse kar ke de raha hai 
    return chotahead; // chotahead is returned
}
